...
def calculate_demographic_data(print_data=True):
    ...
    percentage_bachelors = round((len(df[df.education=="Bachelors"])/(max(df.index)+1))*100,1)
  ...
  # What is the minimum number of hours a person works per week (hours-per-week feature)?
    ...
    for country_name, df2 in df.groupby(['native-country']):
        percentage=len(df2[df2['salary']==">50K"])/len(df2)
    ...
    top_IN_occupation = top_IN_occupation=df.loc[(df.salary==">50K") & (df['native-country']=="India")]

    top_IN_occupation=top_IN_occupation.occupation.mode()
